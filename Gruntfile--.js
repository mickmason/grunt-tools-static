  ...
  //grunt.loadNpmTasks('grunt-contrib-csslint');
	...
	const projectFiles = {
		jsFiles: {
			...
			tempPath: 'development/src/js/temp/' 
  ...
  grunt.initConfig({
    pkg: grunt.file.readJSON('package.json'),
		run: {
			...
			jshint: {
				cmd: 'npx',
				args: ['jshint', projectFiles.jsFiles.src[0]]
			},
			babel: {
				cmd: 'npx',
				args: ['babel', projectFiles.jsFiles.src[0], '--out-dir', projectFiles.jsFiles.tempPath]
			},
			uglify: {
				cmd: 'npx',
				args: ['uglifyjs', projectFiles.jsFiles.tempPath + 'scripts.js', '-o', projectFiles.jsFiles.tempPath + 'scripts.min.js', '-m']
			}
		},
    express: {
      all: {
